            ...
            <div class="card-body">
                ...
                <table id="demo-foo-addrow" class="table table-centered table-striped table-bordered mb-0 toggle-circle default footable-loaded footable" data-page-size="7">
                    ...
                    <tr class="footable-even" style="">
                        <td class="text-center footable-visible footable-first-column">{{ $child->karmand->name }}</td>
                        <td class="text-center footable-visible footable-first-column">{{ $child->name }}</td>
                        <td class="footable-visible"><span class="footable-toggle"></span>{{ $child->family }}</td>
                        <td class="footable-visible">{{ $child->father_name }}</td>
                        <td class="footable-visible">{{ $child->kodmeli }}</td>
                        <td class="footable-visible">{{ $child->birth }}</td>
                        <td class="footable-visible">{{ $child->cellphone }}</td>
                        <td class="footable-visible">
                            <a href="{{ route('admin.children.index') }}" class="btn btn-sm btn-secondary">بازگشت</a>
                        </td>
                    </tr>
                    </tfoot>
                </table>
